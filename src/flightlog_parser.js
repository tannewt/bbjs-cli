...
/* eslint global-strict: true, devel: true, typed: true, browser: true */
"use strict";
...
// This is copied from https://github.com/cleanflight/blackbox-log-viewer/blob/master/js/flightlog_parser.js
...
var FlightLogParser = function(logData) {
    ...
    function parseHeaderLine() {
        ...
        if (that.stream.peekChar() != ' ')
            return;

        //Skip the leading space
        that.stream.readChar();

        lineStart = that.stream.pos;

        for (; that.stream.pos < lineStart + 1024 && that.stream.pos < that.stream.end; that.stream.pos++) {
            if (separatorPos === false && that.stream.data[that.stream.pos] == COLON)
                separatorPos =  that.stream.pos;

            if (that.stream.data[that.stream.pos] == NEWLINE || that.stream.data[that.stream.pos] === 0)
                break;
        }

        if (that.stream.data[that.stream.pos] != NEWLINE || separatorPos === false)
            return;

        lineEnd = that.stream.pos;
        fieldName = tools.asciiArrayToString(that.stream.data.subarray(lineStart, separatorPos));
        fieldValue = tools.asciiArrayToString(that.stream.data.subarray(separatorPos + 1, lineEnd));
        ...
        switch (fieldName) {
            ...
            case "vbatcellvoltage":
                var vbatcellvoltageParams = tools.parseCommaSeparatedIntegers(fieldValue);
            ...
            break;
            case "currentMeter":
                var currentMeterParams = tools.parseCommaSeparatedIntegers(fieldValue);

                that.sysConfig.currentMeterOffset = currentMeterParams[0];
                that.sysConfig.currentMeterScale = currentMeterParams[1];
            break;
            case "gyro.scale":
                that.sysConfig.gyroScale = tools.hexToFloat(fieldValue);
            ...
            default:
                if ((matches = fieldName.match(/^Field (.) (.+)$/))) {
                    ...
                    switch (frameInfo) {
                        case "predictor":
                            frameDef.predictor = tools.parseCommaSeparatedIntegers(fieldValue);
                        break;
                        case "encoding":
                            frameDef.encoding = tools.parseCommaSeparatedIntegers(fieldValue);
                        break;
                        case "name":
                        ...
                        break;
                        case "signed":
                            frameDef.signed = tools.parseCommaSeparatedIntegers(fieldValue);
                        break;
                        default:
            ...
            if (predictor[i] == FLIGHT_LOG_FIELD_PREDICTOR_INC) {
                ...
                switch (encoding[i]) {
                    case FLIGHT_LOG_FIELD_ENCODING_SIGNED_VB:
                        value = that.stream.readSignedVB();
                    break;
                    case FLIGHT_LOG_FIELD_ENCODING_UNSIGNED_VB:
                        value = that.stream.readUnsignedVB();
                    break;
                    case FLIGHT_LOG_FIELD_ENCODING_NEG_14BIT:
                        value = -tools.signExtend14Bit(that.stream.readUnsignedVB());
                    break;
                    case FLIGHT_LOG_FIELD_ENCODING_TAG8_4S16:
                        if (dataVersion < 2)
                            that.stream.readTag8_4S16_v1(values);
                        else
                            that.stream.readTag8_4S16_v2(values);
                    ...
                    case FLIGHT_LOG_FIELD_ENCODING_TAG2_3S32:
                        that.stream.readTag2_3S32(values);
                    ...
                    case FLIGHT_LOG_FIELD_ENCODING_TAG8_8SVB:
                        ...
                        that.stream.readTag8_8SVB(values, groupCount);
    ...
    function parseEventFrame(raw) {
        var
            ...
            eventType = that.stream.readByte();
        ...
        switch (eventType) {
            case FlightLogEvent.SYNC_BEEP:
                lastEvent.data.time = that.stream.readUnsignedVB();
                lastEvent.time = lastEvent.data.time;
            break;
            case FlightLogEvent.AUTOTUNE_CYCLE_START:
                lastEvent.data.phase = that.stream.readByte();

                var cycleAndRising = that.stream.readByte();

                lastEvent.data.cycle = cycleAndRising & 0x7F;
                lastEvent.data.rising = (cycleAndRising >> 7) & 0x01;

                lastEvent.data.p = that.stream.readByte();
                lastEvent.data.i = that.stream.readByte();
                lastEvent.data.d = that.stream.readByte();
            break;
            case FlightLogEvent.AUTOTUNE_CYCLE_RESULT:
                lastEvent.data.overshot = that.stream.readByte();
                lastEvent.data.p = that.stream.readByte();
                lastEvent.data.i = that.stream.readByte();
                lastEvent.data.d = that.stream.readByte();
            break;
            case FlightLogEvent.AUTOTUNE_TARGETS:
                ...
                lastEvent.data.currentAngle = that.stream.readS16() / 10.0;

                lastEvent.data.targetAngle = that.stream.readS8();
                lastEvent.data.targetAngleAtPeak = that.stream.readS8();

                lastEvent.data.firstPeakAngle = that.stream.readS16() / 10.0;
                lastEvent.data.secondPeakAngle = that.stream.readS16() / 10.0;
            break;
            case FlightLogEvent.GTUNE_CYCLE_RESULT:
                lastEvent.data.axis = that.stream.readU8();
                lastEvent.data.gyroAVG = that.stream.readSignedVB();
                lastEvent.data.newP = that.stream.readS16();
            break;
            case FlightLogEvent.INFLIGHT_ADJUSTMENT:
                var tmp = that.stream.readU8();
                lastEvent.data.name = 'Unknown';
                lastEvent.data.func = tmp & 127;
                lastEvent.data.value = tmp < 128 ? that.stream.readSignedVB() : uint32ToFloat(that.stream.readU32());
            ...
            break;
            case FlightLogEvent.LOGGING_RESUME:
                lastEvent.data.logIteration = that.stream.readUnsignedVB();
                lastEvent.data.currentTime = that.stream.readUnsignedVB();
            break;
            case FlightLogEvent.LOG_END:
                var endMessage = that.stream.readString(END_OF_LOG_MESSAGE.length);

                if (endMessage == END_OF_LOG_MESSAGE) {
                    //Adjust the end of that.stream so we stop reading, this log is done
                    that.stream.end = that.stream.pos;
                } else {
                    /*
    ...
    this.parseHeader = function(startOffset, endOffset) {
        ...
        that.stream.start = startOffset === undefined ? that.stream.pos : startOffset;
        that.stream.pos = that.stream.start;
        that.stream.end = endOffset === undefined ? that.stream.end : endOffset;
        that.stream.eof = false;

        mainloop:
        while (true) {
            var command = that.stream.readChar();
            ...
            switch (command) {
                ...
                default:
                    ...
                    if (getFrameType(command)) {
                        that.stream.unreadChar(command);
    ...
    this.parseLogData = function(raw, startOffset, endOffset) {
        ...
        while (true) {
            ...
            if (lastFrameType) {
                var
                    lastFrameSize = that.stream.pos - frameStart,
                    frameTypeStats;
                ...
                if (lastFrameSize <= FLIGHT_LOG_MAX_FRAME_LENGTH && looksLikeFrameCompleted) {
                    ...
                    if (lastFrameType.complete)
                        frameAccepted = lastFrameType.complete(lastFrameType.marker, frameStart, that.stream.pos, raw);
        ...
        this.stats.totalBytes += that.stream.end - that.stream.start;
    ...
    that.stream = new ArrayDataStream(logData);
};
...
FlightLogParser.prototype.FLIGHT_LOG_START_MARKER = tools.asciiStringToByteArray("H Product:Blackbox flight data recorder by Nicholas Sherlock\n");
...
module.exports = FlightLogParser;
